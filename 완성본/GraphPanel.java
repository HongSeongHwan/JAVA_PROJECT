...
public class GraphPanel extends JPanel {
	...
	public GraphPanel(){
		...
		// type - type name like total kcal, protein, fat ..
		// Left Side - small text
		type();
		
		// graph - put on FoodAteBar objects ( had been threading )
		// Middle Side - Graph 
	...
	// Counting the ate data
	public void countD() {	
		// percentage return - real time data from thread
		totP	= tot.getPercent();
		danP	= dan.getPercent();
		tanP	= tan.getPercent();
		jiP		= ji.getPercent();
		
		// write using this real time data
		lblTD.setText(totP+"%");
		lblPD.setText(danP+"%");
		lblCD.setText(tanP+"%");
		lblFD.setText(jiP+"%");
		
		// color  - if percentage over 100 , color -> red
	...
	// if food added to TimeLinePanel, this method will be operated
